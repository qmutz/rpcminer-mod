...
public:
    ...
    const unsigned long GetNumBlocks() const
    {
        return m_numb;
    }

    const unsigned long GetNumThreads() const
    {
        return m_numt;
    }

    const DEVICECOUNTTYPE GetDeviceCount() const
    {
        return m_devicecount;
    }

    const unsigned int GetStepIterations() const
    {
        return (1u << (m_bits - 1));
    }

    const unsigned int GetStepBitShift() const
    {
        return m_bits;
    }
    ...
    virtual const STEPTYPE RunStep(uint32 nonce) = 0;

    void SetGrid(const int grid)
    {
        m_numb = grid;
    }
...
protected:
    ...
    virtual const bool AllocateResources(const int numb, const int numt) = 0;
